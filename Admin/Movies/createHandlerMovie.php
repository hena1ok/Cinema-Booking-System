...
if(isset($_POST['submit'])&&isset($_FILES['video'])||isset($_FILES['image'])){
    ...
    if(in_array($video_ex_lc,$allowed_exs)&&in_array($imgExtentionlc,$allowedimgs)){
        if($videoSize<10000000000&&$imgSize<500000000){
            ...
            $sql = "insert into movies (`Id`,`Title`,`Release_date`,`Category`,`Runtime`,`Country`,`Image`,`Trailer`)  values ('$id','$title','$release_date','$category','$runtime','$country','$img_name','$video_name')";
        ...
        header("Location: Read.php?page=create");
